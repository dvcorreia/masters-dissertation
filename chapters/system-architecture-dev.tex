...
From the product cases, GS1-128 barcode, and sleeves, \acs{ean}-13 barcode, used for testing, two Nespresso company prefixes were present: $76300544$ and $76300396$ respectively. Searching in GS1 Company Database (GEPIR), it was found $18$ more company prefixes registered by Nestlé Nespresso SA in Switzerland.
From the \acs{ean}-13 barcode in sleeves, the \acs{gtin}-13 can be inferred and converted to \acs{gtin}-14, covered in section~\ref{sec:sgtin}, used in \ac{sgtin}-96 encoding for \ac{gen2} \ac{epc} \acs{rfid} tags.
For the GS1-128 barcode of product cases for transport, an example of the deconstruction of the barcode is illustrated in figure~\ref{fig:gs1-128barcode}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figs/gs1-128barcode.pdf}
    \caption{Deconstruction of a GS1-128 barcode from a Volluto cardboard case for transport used in this dissertation has a test product}
    \label{fig:gs1-128barcode}
\end{figure}

With the \acs{gtin}-14, it is possible to encode it in \ac{gen2} tags, using \ac{sgtin}-96 encoding. To write the \acp{epc} in the testing tags, it was used the Impinj Octane Java SKD \texttt{WriteEpc.java} sample code was adapted. Instead of generating a random \ac{epc}, the Fosstrak \ac{tdt} engine was used to encode a valid \ac{epc} with the correspondent company prefix, item reference and serial in to hexadecimal, which can be passed to the \texttt{TagWriteOp} class to configure the reader to program the tag.
