...
\paragraph*{\ac{uhf}} operates in the band of $315$ to $433MHz$~\cite{isoISOIEC180007}, when using active tags, and $860$ to $956$~\cite{isoISOIEC180006} using passive ones. It is known for the promises it brings to \ac{scm} and logistics - a cheap and affordable technology to wirelessly identify and track every item. It is mainly used in item tracking but it can be used for all kinds of applications.I uses backscatter coupling to transmit data between reader and tag. These frequencies present some drawbacks: poor behavior on liquid and metals is a big problem with passive tags; the allocation of the \ac{rf} spectrum differs between countries being a huge problem for global deployments. This thematic will be further discussed throughout this dissertation.

\paragraph*{Microwave} typically operates in $2.45GHz$~\cite{isoISOIEC180004} ($5.8GHz$ was withdrawn).


For concise information related to each group, consult table~\ref{tab:} for \ac{rf} properties in different materials and tables~\ref{tab:} for an abridgment of much general information.
...
% See: 1.1.1 Type of tags on D'Alessandro
